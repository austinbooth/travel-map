...
const LocationForm: FC<LocationFormProps> = ({setCoords}) => {
  ...
  const clearFormAndChangeMapLocation = async (location: string) => {
    setInputLocation("")
    const {lat, lng} = await api.getLatLngFromName(location)
    setCoords(lat, lng)
  }
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const autoCompleteLocations = ["Prague", "Berlin", "Paris", "St Petersburg", "Split"]
    const possibleOptions = autoCompleteLocations.filter((location) =>
    ...
    if (entered.length === 0) {
      ...
      setSuggestions(possibleOptions)
    }
